...
const quickSaleItemsState = {
  items: [],
  pendingCheckItemId: null,
};

const quickSaleElements = {
  codeInput: clientQuickSaleForm?.querySelector('[data-role="quick-sale-manual-code"]') || null,
  descriptionInput:
    clientQuickSaleForm?.querySelector('[data-role="quick-sale-manual-description"]') || null,
  quantityInput: clientQuickSaleForm?.querySelector('[data-role="quick-sale-manual-quantity"]') || null,
  addButton: clientQuickSaleForm?.querySelector('[data-role="quick-sale-add-manual-code"]') || null,
  checkButton: clientQuickSaleForm?.querySelector('[data-role="quick-sale-check-manual-code"]') || null,
  itemsBody: clientQuickSaleForm?.querySelector('[data-role="quick-sale-items-body"]') || null,
  emptyRow: clientQuickSaleForm?.querySelector('[data-role="quick-sale-items-empty"]') || null,
};

const quickSaleCheckOverlay = document.querySelector('[data-modal="quick-sale-check-item"]');
const quickSaleCheckForm = quickSaleCheckOverlay?.querySelector('[data-role="quick-sale-check-form"]') || null;
const quickSaleCheckInput = quickSaleCheckOverlay?.querySelector('[data-role="quick-sale-check-input"]') || null;
const quickSaleCheckFeedback =
  quickSaleCheckOverlay?.querySelector('[data-role="quick-sale-check-feedback"]') || null;
const quickSaleCheckSubmitButton =
  quickSaleCheckOverlay?.querySelector('[data-role="quick-sale-check-submit"]') || null;
const quickSaleCheckConfirmButton =
  quickSaleCheckOverlay?.querySelector('[data-role="quick-sale-check-confirm"]') || null;
const quickSaleCheckDismissButton =
  quickSaleCheckOverlay?.querySelector('[data-role="quick-sale-check-dismiss"]') || null;
const quickSaleCheckCloseButton =
  quickSaleCheckOverlay?.querySelector('[data-action="close-quick-sale-check"]') || null;
const quickSaleCheckCancelButton =
  quickSaleCheckOverlay?.querySelector('[data-action="cancel-quick-sale-check"]') || null;

const quickSaleEditOverlay = document.querySelector('[data-modal="quick-sale-edit-item"]');
const quickSaleEditForm = quickSaleEditOverlay?.querySelector('[data-role="quick-sale-edit-form"]') || null;
const quickSaleEditIdInput = quickSaleEditOverlay?.querySelector('[data-role="quick-sale-edit-id"]') || null;
const quickSaleEditCodeInput =
  quickSaleEditOverlay?.querySelector('[data-role="quick-sale-edit-code"]') || null;
const quickSaleEditDescriptionInput =
  quickSaleEditOverlay?.querySelector('[data-role="quick-sale-edit-description"]') || null;
const quickSaleEditQuantityInput =
  quickSaleEditOverlay?.querySelector('[data-role="quick-sale-edit-quantity"]') || null;
const quickSaleEditSaveButton =
  quickSaleEditOverlay?.querySelector('[data-action="save-quick-sale-edit"]') || null;
const quickSaleEditCancelButton =
  quickSaleEditOverlay?.querySelector('[data-action="cancel-quick-sale-edit"]') || null;
const quickSaleEditCloseButton =
  quickSaleEditOverlay?.querySelector('[data-action="close-quick-sale-edit"]') || null;
...
(function normalizeClientData() {
  ...
  CLIENTS.forEach((client, index) => {
    if (client) {
      ...
      const digits =
        typeof window.normalizePhoneDigits === 'function'
          ? window.normalizePhoneDigits(rawPhone)
          : rawPhone.replace(/\D/g, '');
      client.phone = rawPhone;
      client.phoneDigits = digits;
    }
...
(function initializeClientsPage() {
  ...
  function formatClientPhone(value, digitsHint = null) {
    ...
    const digits =
      digitsHint !== null && digitsHint !== undefined
        ? String(digitsHint)
    ...
    if (typeof window.formatPhoneNumber === 'function') {
      try {
        const formatted = window.formatPhoneNumber(digits);
        return formatted || digits;
      } catch (error) {
        // Ignore formatting errors and fall back to digits below.
      }
    }
  ...
  function formatPhoneForInputValue(value, digitsHint = null) {
    ...
    if (typeof window.formatPhoneForInput === 'function') {
      try {
        return window.formatPhoneForInput(digits);
      } catch (error) {
        // Ignore formatting errors and fall back to digits below.
      }
    }
    ...
    if (typeof window.formatPhoneNumber === 'function') {
      try {
        const formatted = window.formatPhoneNumber(digits);
        return formatted === 'Não informado' ? digits : formatted;
      } catch (error) {
        // Ignore formatting errors and fall back to digits below.
      }
    }
  ...
  function getFilteredClients() {
    ...
    return CLIENTS.filter((client) => {
      ...
      if (phoneFilterDigits) {
        const clientDigits = client.phoneDigits ?? normalizePhoneDigits(client.phone);
        if (!clientDigits.includes(phoneFilterDigits)) {
          return false;
        }
      } else if (phoneFilterText) {
        const formattedPhone = formatClientPhone(client.phone, client.phoneDigits);
  ...
  function createClientRowElement(client) {
    ...
    row.append(
      ...
      createTextCell('phone', formatClientPhone(client.phone, client.phoneDigits)),
  ...
  function mapApiClient(apiClient) {
    ...
    const phoneSource =
      ...
      '';
    const phoneText = phoneSource === undefined || phoneSource === null ? '' : String(phoneSource).trim();
    const phoneDigits = normalizePhoneDigits(phoneText);
  ...
  function renderClientDetail(client) {
    ...
    const detailMap = {
      name: client.name,
      cpf: client.cpf,
      phone: formatClientPhone(client.phone, client.phoneDigits),
      gender: genderLabel,
  ...
  function notifyQuickSale(message, { type = 'info' } = {}) {
    if (!message) {
      return;
    }
    if (typeof window.showToast === 'function') {
      ...
      console.info('[quick-sale]', message);
    }
  }

  function generateQuickSaleItemId() {
    return `qs-item-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  }
  ...
  function resetQuickSaleManualInputs() {
    if (quickSaleElements.codeInput) {
      quickSaleElements.codeInput.value = '';
    }
    if (quickSaleElements.descriptionInput) {
      quickSaleElements.descriptionInput.value = '';
    }
    if (quickSaleElements.quantityInput) {
      quickSaleElements.quantityInput.value = '1';
    }
  }

  function findQuickSaleItemById(itemId) {
    if (!itemId) {
      return null;
    }
    const normalized = String(itemId);
    return quickSaleItemsState.items.find((item) => item.id === normalized) || null;
  }

  function findQuickSaleItemByCode(normalizedCode, { excludeId = null } = {}) {
    if (!normalizedCode) {
      return null;
    }
    return (
      quickSaleItemsState.items.find((item) => {
        if (excludeId && item.id === excludeId) {
          return false;
        }
        return item.normalizedCode === normalizedCode;
      }) || null
    );
  }

  function renderQuickSaleItems() {
    const { itemsBody, emptyRow } = quickSaleElements;
    if (!itemsBody) {
      return;
    }

    Array.from(itemsBody.querySelectorAll('tr[data-item-id]')).forEach((row) => {
      row.remove();
    });

    if (quickSaleItemsState.items.length === 0) {
      if (emptyRow) {
    ...
    quickSaleItemsState.items.forEach((item) => {
      const row = document.createElement('tr');
      ...
      quantityCell.textContent = String(item.quantity ?? 0);
      row.appendChild(quantityCell);

      const actionsCell = document.createElement('td');
      actionsCell.className = 'quick-sale-items__column-actions';
      const actionsWrapper = document.createElement('div');
      actionsWrapper.className = 'quick-sale-items__row-actions';

      const editButton = document.createElement('button');
      editButton.type = 'button';
      editButton.className = 'quick-sale-items__action-button';
      ...
      deleteButton.className = 'quick-sale-items__action-button quick-sale-items__action-button--danger';
  ...
  function resetQuickSaleItemsState() {
    quickSaleItemsState.items = [];
    renderQuickSaleItems();
    resetQuickSaleManualInputs();
  }

  function addQuickSaleItem({ code, description, quantity }) {
    const normalizedCode = normalizeManualCode(code);
    const trimmedDescription = description === undefined || description === null ? '' : String(description).trim();
    const item = {
      id: generateQuickSaleItemId(),
      code: code ? String(code).trim() : '',
      description: trimmedDescription,
      quantity,
      normalizedCode,
    };
    quickSaleItemsState.items.push(item);
    renderQuickSaleItems();
  }

  function updateQuickSaleItem(itemId, patch = {}) {
    if (!itemId) {
      return;
    }
    const index = quickSaleItemsState.items.findIndex((item) => item.id === itemId);
    if (index === -1) {
      return;
    }
    const current = quickSaleItemsState.items[index];
    const nextCode = patch.code !== undefined ? String(patch.code).trim() : current.code;
    const nextDescription =
      patch.description !== undefined ? String(patch.description).trim() : current.description;
    const nextQuantity = patch.quantity !== undefined ? patch.quantity : current.quantity;

    quickSaleItemsState.items[index] = {
      ...current,
      ...patch,
      code: nextCode,
      description: nextDescription,
      quantity: nextQuantity,
      normalizedCode: normalizeManualCode(nextCode),
    };
    renderQuickSaleItems();
  }

  function removeQuickSaleItem(itemId) {
    if (!itemId) {
      return;
    }
    quickSaleItemsState.items = quickSaleItemsState.items.filter((item) => item.id !== itemId);
    renderQuickSaleItems();
  }

  function openQuickSaleEditModal(itemId) {
    const item = findQuickSaleItemById(itemId);
    if (!item || !quickSaleEditOverlay || !quickSaleEditForm) {
      return;
    }

    if (quickSaleEditIdInput) {
      quickSaleEditIdInput.value = item.id;
    }
    if (quickSaleEditCodeInput) {
      quickSaleEditCodeInput.value = item.code;
    }
    if (quickSaleEditDescriptionInput) {
      quickSaleEditDescriptionInput.value = item.description;
    }
    if (quickSaleEditQuantityInput) {
      quickSaleEditQuantityInput.value = String(item.quantity ?? 1);
    }

    openOverlay(quickSaleEditOverlay);

    window.requestAnimationFrame(() => {
      try {
        quickSaleEditCodeInput?.focus();
      } catch (error) {
        quickSaleEditCodeInput?.focus();
      }
    });
  ...
  function closeQuickSaleEditModal() {
    if (!quickSaleEditOverlay) {
      return;
    }
    quickSaleEditForm?.reset();
    if (quickSaleEditIdInput) {
      quickSaleEditIdInput.value = '';
    }
    closeOverlay(quickSaleEditOverlay);
  }

  function handleQuickSaleEditSave() {
    if (!quickSaleEditForm) {
      return;
    }

    const formData = new FormData(quickSaleEditForm);
    const itemId = String(formData.get('itemId') || '').trim();
    if (!itemId) {
      closeQuickSaleEditModal();
      return;
    }

    const codeValue = String(formData.get('editCode') || '').trim();
    const descriptionValue = String(formData.get('editDescription') || '').trim();
    const quantityValue = Number.parseInt(String(formData.get('editQuantity') || '1'), 10);

    if (!codeValue) {
      notifyQuickSale('Informe um código válido para o item.', { type: 'warning' });
      quickSaleEditCodeInput?.focus();
      return;
    }

    if (!descriptionValue) {
      notifyQuickSale('Informe a descrição do item.', { type: 'warning' });
      quickSaleEditDescriptionInput?.focus();
      return;
    }

    if (!Number.isFinite(quantityValue) || quantityValue <= 0) {
      notifyQuickSale('Informe uma quantidade válida.', { type: 'warning' });
      quickSaleEditQuantityInput?.focus();
      return;
    }

    const normalizedCode = normalizeManualCode(codeValue);
    const existing = findQuickSaleItemByCode(normalizedCode, { excludeId: itemId });
    if (existing) {
      notifyQuickSale('Este código já está cadastrado em outro item.', { type: 'warning' });
      quickSaleEditCodeInput?.focus();
      return;
    }

    updateQuickSaleItem(itemId, {
      code: codeValue,
      description: descriptionValue,
      quantity: quantityValue,
    });

    notifyQuickSale('Item atualizado com sucesso.', { type: 'success' });
    closeQuickSaleEditModal();
  }

  function handleQuickSaleAddClick() {
    const codeValue = quickSaleElements.codeInput ? quickSaleElements.codeInput.value.trim() : '';
    const descriptionValue = quickSaleElements.descriptionInput
      ? quickSaleElements.descriptionInput.value.trim()
      : '';
    const quantityValue = quickSaleElements.quantityInput
      ? Number.parseInt(quickSaleElements.quantityInput.value, 10)
      : 1;

    if (!codeValue) {
      notifyQuickSale('Informe um código para adicionar o item.', { type: 'warning' });
      quickSaleElements.codeInput?.focus();
      return;
    }

    if (!descriptionValue) {
      notifyQuickSale('Informe a descrição do item.', { type: 'warning' });
      quickSaleElements.descriptionInput?.focus();
      return;
    }

    if (!Number.isFinite(quantityValue) || quantityValue <= 0) {
      notifyQuickSale('Informe uma quantidade válida.', { type: 'warning' });
      quickSaleElements.quantityInput?.focus();
      return;
    }

    const normalizedCode = normalizeManualCode(codeValue);
    const duplicated = findQuickSaleItemByCode(normalizedCode);
    if (duplicated) {
      notifyQuickSale('Produto já cadastrado. Utilize o botão Conferir para editá-lo.', {
        type: 'info',
      });
      return;
    }

    addQuickSaleItem({ code: codeValue, description: descriptionValue, quantity: quantityValue });
    notifyQuickSale('Item adicionado à venda.', { type: 'success' });
    resetQuickSaleManualInputs();
    quickSaleElements.codeInput?.focus();
  }

  function handleQuickSaleItemsTableClick(event) {
    ...
    if (action === 'edit-item') {
      openQuickSaleEditModal(itemId);
      return;
    }

    if (action === 'delete-item') {
      const item = findQuickSaleItemById(itemId);
      const confirmationMessage = item?.code
        ? `Deseja remover o item ${item.code}?`
        : 'Deseja remover este item?';
      const confirmed = typeof window.confirm === 'function' ? window.confirm(confirmationMessage) : true;
      if (confirmed) {
        removeQuickSaleItem(itemId);
        notifyQuickSale('Item removido da venda.', { type: 'info' });
      }
    }
  }

  function setQuickSaleCheckMessage(message, variant = '') {
    if (!quickSaleCheckFeedback) {
      return;
    }
    quickSaleCheckFeedback.textContent = message || '';
    quickSaleCheckFeedback.classList.remove('is-error', 'is-success');
    if (variant === 'error') {
      quickSaleCheckFeedback.classList.add('is-error');
    } else if (variant === 'success') {
      quickSaleCheckFeedback.classList.add('is-success');
    }
  }

  function resetQuickSaleCheckModal({ presetCode = '' } = {}) {
    quickSaleItemsState.pendingCheckItemId = null;
    if (quickSaleCheckInput) {
      quickSaleCheckInput.value = presetCode;
      if (presetCode) {
        quickSaleCheckInput.select();
      }
    }
    setQuickSaleCheckMessage('');
    if (quickSaleCheckSubmitButton) {
      quickSaleCheckSubmitButton.hidden = false;
    }
    if (quickSaleCheckConfirmButton) {
      quickSaleCheckConfirmButton.hidden = true;
      delete quickSaleCheckConfirmButton.dataset.itemId;
    }
    if (quickSaleCheckDismissButton) {
      quickSaleCheckDismissButton.hidden = true;
    }
  }

  function openQuickSaleCheckModal() {
    if (!quickSaleCheckOverlay) {
      return;
    }

    const presetCode = quickSaleElements.codeInput ? quickSaleElements.codeInput.value.trim() : '';
    resetQuickSaleCheckModal({ presetCode });
    openOverlay(quickSaleCheckOverlay);

    window.requestAnimationFrame(() => {
      try {
        quickSaleCheckInput?.focus();
      } catch (error) {
        quickSaleCheckInput?.focus();
      }
    });
  ...
  function closeQuickSaleCheckModal() {
    if (!quickSaleCheckOverlay) {
      return;
    }
    resetQuickSaleCheckModal();
    closeOverlay(quickSaleCheckOverlay);
  }

  function handleQuickSaleCheckSubmit() {
    if (!quickSaleCheckInput) {
      return;
    }
    const codeValue = quickSaleCheckInput.value.trim();
    if (!codeValue) {
      setQuickSaleCheckMessage('Informe um código para realizar a conferência.', 'error');
      quickSaleCheckInput.focus();
      return;
    }

    const normalized = normalizeManualCode(codeValue);
    const item = findQuickSaleItemByCode(normalized);
    if (!item) {
      quickSaleItemsState.pendingCheckItemId = null;
      setQuickSaleCheckMessage('Produto não cadastrado na venda.', 'error');
      if (quickSaleCheckSubmitButton) {
        quickSaleCheckSubmitButton.hidden = false;
      }
      if (quickSaleCheckConfirmButton) {
    ...
    quickSaleItemsState.pendingCheckItemId = item.id;
    setQuickSaleCheckMessage('Produto já cadastrado. Gostaria de editá-lo?', 'success');
    if (quickSaleCheckSubmitButton) {
      quickSaleCheckSubmitButton.hidden = true;
    }
    if (quickSaleCheckConfirmButton) {
      quickSaleCheckConfirmButton.hidden = false;
      quickSaleCheckConfirmButton.dataset.itemId = item.id;
    }
    if (quickSaleCheckDismissButton) {
      quickSaleCheckDismissButton.hidden = false;
    }

    window.requestAnimationFrame(() => {
      try {
        quickSaleCheckConfirmButton?.focus();
      } catch (error) {
        quickSaleCheckConfirmButton?.focus();
      }
    });
  ...
  function handleQuickSaleCheckConfirm() {
    const itemId = quickSaleItemsState.pendingCheckItemId || quickSaleCheckConfirmButton?.dataset.itemId;
    if (!itemId) {
      closeQuickSaleCheckModal();
      return;
    }
    closeQuickSaleCheckModal();
    openQuickSaleEditModal(itemId);
  ...
  function handleQuickSaleCheckDismiss() {
    closeQuickSaleCheckModal();
  }

  function handleQuickSaleCheckFormSubmit(event) {
    event.preventDefault();
    handleQuickSaleCheckSubmit();
  }

  function handleQuickSaleCheckOverlayClick(event) {
    if (!(event.target instanceof Element)) {
      return;
    }
    if (!event.target.closest('.modal')) {
  ...
  function handleQuickSaleEditOverlayClick(event) {
    if (!(event.target instanceof Element)) {
      return;
    }
    if (!event.target.closest('.modal')) {
  ...
  function collectQuickSaleFormData() {
    ...
    const items = quickSaleItemsState.items.map((item) => ({
      id: item.id,
      code: item.code,
  ...
  function updateClientRow(client) {
    ...
    updateText('phone', formatClientPhone(client.phone, client.phoneDigits));
  ...
  function prepareClientForm(mode = 'create', client = null) {
    ...
    const fields = {
      name: client?.name ?? '',
      cpf: client?.cpf ?? '',
      phone: formatPhoneForInputValue(client?.phone ?? '', client?.phoneDigits ?? null),
  ...
  function collectFormData() {
    ...
    const phoneText = rawPhone === null ? '' : rawPhone.toString().trim();
    const phoneDigits = normalizePhoneDigits(phoneText);
  ...
  attachButtonHandler(quickSaleElements.addButton, handleQuickSaleAddClick);
  attachButtonHandler(quickSaleElements.checkButton, openQuickSaleCheckModal);
  quickSaleElements.itemsBody?.addEventListener('click', handleQuickSaleItemsTableClick);
  quickSaleCheckForm?.addEventListener('submit', handleQuickSaleCheckFormSubmit);
  attachButtonHandler(quickSaleCheckSubmitButton, handleQuickSaleCheckSubmit);
  attachButtonHandler(quickSaleCheckConfirmButton, handleQuickSaleCheckConfirm);
  attachButtonHandler(quickSaleCheckDismissButton, handleQuickSaleCheckDismiss);
  attachButtonHandler(quickSaleCheckCloseButton, closeQuickSaleCheckModal);
  attachButtonHandler(quickSaleCheckCancelButton, closeQuickSaleCheckModal);
  quickSaleCheckOverlay?.addEventListener('click', handleQuickSaleCheckOverlayClick);
  quickSaleEditForm?.addEventListener('submit', (event) => {
    event.preventDefault();
    handleQuickSaleEditSave();
  });
  attachButtonHandler(quickSaleEditSaveButton, handleQuickSaleEditSave);
  attachButtonHandler(quickSaleEditCancelButton, closeQuickSaleEditModal);
  attachButtonHandler(quickSaleEditCloseButton, closeQuickSaleEditModal);
  quickSaleEditOverlay?.addEventListener('click', handleQuickSaleEditOverlayClick);
  ...
  const clientFormPhoneInput = clientFormElement
    ...
    : null;
  if (clientFormPhoneInput instanceof HTMLInputElement) {
    clientFormPhoneInput.setAttribute('inputmode', 'tel');
    clientFormPhoneInput.setAttribute('autocomplete', 'tel');
  }
